...
(function(jQuery){
	...
	jQuery.fn.extend({
		...
		apply: function( data ) {
			return jQuery.apply( this.html(), data );
    },
    ...
    render: function( tmpl, data ) {
      ...
      //copy the required variables from global to local scope
      var tmpArr = jQuery.localizeGlobalScope(tmpl, data);
      data = tmpArr[1];
      tmpl = tmpArr[0];
    ...
    apply: function( tmpl, data ) {
      ...
      "times": {
        prefix: "for(var $1=0;$1<$2;$1++){with(this){",
        suffix: "}}"
      },
      "func": {
        prefix: "_.push($2.call(this, $1));"
      },
			"if": {
				prefix: "if($1){",
				suffix: "}"
    ...
    localizeGlobalScope: function(tmpl, data) {
      var str = (typeof(tmpl) === "string") ? tmpl : tmpl.innerHTML;

      //replace direct references for ex. ${#globalVarName}
      str = str.replace(/\$\{\#([^}]*)}/g, function(all, varName){
        return "${" + jQuery.performLocalization(data, varName) + "}"; 
      });

      //replace references as function arguments for ex. {{if (#globalVar)}}
      var calls = str.match(/\{\{(.*?)(?: \(([^']*?\#.*?))}}/g);
      
      if (calls) {
        for (var i = 0; i < calls.length; i++) {
          str = str.replace(calls[i], calls[i].replace(/\#([^\s]*)/g, function(all, varName){
            return jQuery.performLocalization(data, varName);  
          }));
        }
      }
    ...
    performLocalization: function(data, varName) {
      var randVarName = "__" + MEM.tmplCounter + "varName";
      data[randVarName] = eval(varName);
      MEM.tmplCounter += 1;
      return randVarName;
    }, 
    ...
    tmpl: function(str, data, i) {
      ...
      var a = data ? fn.call( this, jQuery, data, i ) : fn;
      return a;
		}
	});
})(jQuery);
